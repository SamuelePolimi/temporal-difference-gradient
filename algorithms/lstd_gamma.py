...
class LSTDGammaNew(PolicyGradient, Critic):
    def __init__(self, policy: Union[RLAgent, RLParametricModel], critic_features, actor_features, dataset: RLDataset,
                 ...
                 reparametrization=False, regularization=1E-12, delayed=True):
        """
    ...
    def get_B(self, batch):
        # TODO: check
        s, a, r, s_n, t = [torch.tensor(b, dtype=torch.float64) for b in batch]
        n = s.shape[0]

        A, b, _ = self.get_A_b(batch)
        omega = torch.linalg.inv(A + self._regularization * torch.eye(A.shape[0]))

        def Q(state, action):
            phi = self._critic_features(state.expand(1).int(), action.expand(1).int())
            return torch.inner(phi.squeeze(), omega)

        B_ret = 0.
        for s_i, a_i, s_i_n in zip(s, a, s_n):
            phi = self._critic_features(s_i.expand(1).int(), a_i.expand(1).int())
            phi = phi.squeeze()
            if self._delayed:
                a_n = self.policy(s_i_n.expand(1), differentiable=True)
                grad = Q(s_i_n, a_n) * self.get_nabla_log(s_i_n, a_n)
            else:
                grad = Q(s_i, a_i) * self.get_nabla_log(s_i, a_i)
            B_ret += torch.outer(phi, torch.tensor(grad))
